...
from campnet.configuration import build_restore_plan
from campnet.radio import ModemPreference


def test_build_restore_plan_maps_response_names_to_registry_operations() -> None:
    preferences = (
    ...
    plan = build_restore_plan(preferences)

    assert tuple(operation.command_id for operation in plan) == (
        "config.set_mode_preference",
        "config.set_rat_order",
        "config.set_lte_bands",
    )
    assert tuple(operation.render() for operation in plan) == (
...
def test_restore_plan_rejects_unexpected_characters() -> None:
    ...
    try:
        build_restore_plan(preferences)
